...
export function updateNode(
  nodeData: ClusterViewNode,
  nodeStat: ClusterViewMessage | ClusterViewItemResponse,
  isRest?: boolean,
): ClusterViewNode | void {
  const node = {
    ...nodeData,
  };
  ...
  if ((nodeStat as ClusterViewItemResponse).workloads?.length) {
    node.workloads = createWorkloads(
      new Map(),
      (nodeStat as ClusterViewItemResponse).workloads as WorkloadsResponseInfo[],
    );
  }
...
export function addNode(
  ...
  if ((nodeStat as ClusterViewItemResponse).workloads?.length) {
    newNode.workloads = createWorkloads(
      new Map(),
      (nodeStat as ClusterViewItemResponse).workloads as WorkloadsResponseInfo[],
    );
  }
...
export function setWorkloads(
  workloads: Map<string, Workload>,
  newWorkload: WorkloadsMessageInfo,
): Map<string, Workload> {
...
function createWorkloads(
  workloads: Map<string, Workload>,
  newWorkloads: Array<WorkloadsResponseInfo>,
): Map<string, Workload> {
...
export function memoryUsage(state: DeviceInfo['state'], timestamp: number): MemoryUsage {
  ...
  const vmMem =
    state.vm?.system?.ram && state.vm?.load?.allocated
      ? formatMBytes(state.vm.system.ram * state.vm.load.allocated)
      : 0;
...
export const filterOutAbsentData = <TData>(
  initData: Map<string, TData>,
  updatedData: ClusterViewResponse,
): Map<string, TData> => {
