...
const SystemCapacityCard: React.FC<Props> = ({ nodes }: Props) => {
  ...
  const systemCPU = useMemo(() => {
    return nodes.reduce(
      (acc, node: NodeItemResponse) => {
        if (node.is_connected) {
          return [{ ...acc[0], count: acc[0].count + toNumber(node.vm?.cpu) }, acc[1]];
        }
        return [acc[0], { ...acc[1], count: acc[1].count + toNumber(node.vm?.cpu) }];
      },
      [
  ...
  const systemMem = useMemo(() => {
    return nodes
      .reduce(
        (acc, node: NodeItemResponse) => {
          if (node.is_connected) {
            return [{ ...acc[0], count: acc[0].count + formatMBytes(node.vm?.ram || 0) }, acc[1]];
          }
          return [acc[0], { ...acc[1], count: acc[1].count + formatMBytes(node.vm?.ram || 0) }];
        },
        [
          { count: 0, name: 'Online', fill: '#B6ED8B' },
          { count: 0, name: 'Offline', fill: '#E2E2E2' },
  ...
  return (
    <Card
      header={
        <Typography variant="h5" fontWeight={700} color="secondary" display="inline-block">
